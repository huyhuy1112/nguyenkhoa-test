	...
	function process (Vtiger_Request $request) {
		...
		try {
			...
			if ($handler) {
				...
				// Heartbeat Team Status: mỗi lần user mở trang (không Ajax) cập nhật login_time để coi là đang hoạt động (30 phút)
				if (!$request->isAjax()) {
					$userName = null;
					if (class_exists('Users_Record_Model')) {
						$cu = Users_Record_Model::getCurrentUserModel();
						if ($cu) {
							$userName = $cu->get('user_name');
						}
					}
					if (empty($userName)) {
						$user = $this->getLogin();
						if ($user && !empty($user->column_fields['user_name'])) {
							$userName = $user->column_fields['user_name'];
						}
					}
					if (!empty($userName)) {
						require_once 'include/utils/LoginHeartbeat.php';
						LoginHeartbeat::update($userName);
					}
				}
