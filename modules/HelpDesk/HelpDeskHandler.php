	...
	function handleEvent($eventName, $entityData) {
		...
		if($eventName == 'vtiger.entity.aftersave.final') {
			...
			if ($moduleName == 'HelpDesk') {
				...
				// Create notification when ticket is assigned to a user
				try {
					$recordId = $ticketId;
					if (empty($recordId)) {
						return;
					...
					$changes = $delta->getEntityDelta('HelpDesk', $recordId);
					...
					if (empty($ticketTitle)) {
						...
						$nameResult = $adb->pquery("SELECT title FROM vtiger_troubletickets WHERE ticketid = ?", array($recordId));
						if ($adb->num_rows($nameResult) > 0) {
							$ticketTitle = $adb->query_result($nameResult, 0, 'title');
						}
					}
					if (empty($ticketTitle)) {
						$ticketTitle = 'Ticket #' . $recordId;
					}
					...
					if ($log) {
						$log->error("[HelpDeskHandler] Error creating notification: " . $e->getMessage());
					}
				}
			}
		}
