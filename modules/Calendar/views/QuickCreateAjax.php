...
class Calendar_QuickCreateAjax_View extends Vtiger_QuickCreateAjax_View {
	...
	public function  process(Vtiger_Request $request) {
		...
		// Build fieldsInfo from the requested module's fields (not last iteration)
		$requestedModuleModel = Vtiger_Module_Model::getInstance($moduleName);
		$requestedFieldList = $requestedModuleModel->getFields();
		$fieldsInfo = array();
		foreach($requestedFieldList as $name => $model){
		...
		// Add Calendar QuickCreate enhancements JS (safe client-side only)
		$scripts = $this->getHeaderScripts($request);
		if (in_array($moduleName, array('Calendar', 'Events'))) {
			$calendarScript = new Vtiger_JsScript_Model();
			$calendarScript->set('src', 'layouts/v7/modules/Calendar/resources/ScheduleQuickCreate.js');
			$calendarScript->set('type', 'text/javascript');
