<?php
/*+***********************************************************************************
 * Export PHIẾU NHẬP KHO — HTML preview in iframe, PDF download separately.
 *************************************************************************************/

require_once 'modules/Warehouse/models/WhMgmtService.php';
require_once 'modules/Warehouse/helpers/InboundReceiptPdf.php';
...
class Warehouse_ExportInboundPDF_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		$receiptCode = trim((string) $request->get('record'));
		if ($receiptCode === '') {
			$receiptCode = trim((string) $request->get('code'));
		}
		...
		try {
			...
			$payload = Warehouse_WhMgmtService::getInboundReceiptPrintPayload($warehouseCode, $receiptCode);
			...
			if ($isPreview || $format === 'html') {
				Warehouse_InboundReceiptPdf_Helper::renderHtmlPreview($payload);
			}
			...
			$fileName = 'PhieuNhapKho_' . preg_replace('/[^A-Za-z0-9_-]+/', '_', $receiptCode) . '.pdf';
			Warehouse_InboundReceiptPdf_Helper::output($payload, $fileName, 'D');
			exit;
		} catch (Exception $e) {
			...
			echo '<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Lỗi in phiếu</title></head><body style="font-family:sans-serif;padding:24px;color:#b91c1c">'
				. '<h3>Không tải được phiếu nhập kho</h3><p>'
