<?php
/*+***********************************************************************************
 * Export PHIẾU THU HỒI / TRẢ HÀNG — HTML preview in iframe, PDF download separately.
 *************************************************************************************/

require_once 'modules/Warehouse/models/WhMgmtService.php';
require_once 'modules/Warehouse/helpers/ReturnHelper.php';
require_once 'modules/Warehouse/helpers/ReturnPdf.php';

if (!class_exists('TCPDF')) {
	$__whAppRoot = dirname(__DIR__, 3);
...
class Warehouse_ExportReturnPDF_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		$code = trim((string) $request->get('record'));
		if ($code === '') {
		...
		try {
			if ($warehouseCode === '' || $code === '') {
				throw new Exception('Thiếu mã kho hoặc mã phiếu thu hồi.');
			}
			Warehouse_WhMgmtService::ensureInstalled();
			$payload = Warehouse_Return_Helper::printPayload($warehouseCode, $code);
			...
			if ($isPreview || $format === 'html') {
				Warehouse_ReturnPdf_Helper::renderHtmlPreview($payload);
			}
			...
			$fileName = 'PhieuThuHoi_' . preg_replace('/[^A-Za-z0-9_-]+/', '_', $code) . '.pdf';
			Warehouse_ReturnPdf_Helper::output($payload, $fileName, 'D');
			exit;
		} catch (Exception $e) {
			...
			echo '<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Lỗi in phiếu</title></head><body style="font-family:sans-serif;padding:24px;color:#b91c1c">'
				. '<h3>Không tải được phiếu thu hồi / trả hàng</h3><p>'
