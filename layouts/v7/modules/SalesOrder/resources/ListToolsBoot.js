/**
 * SalesOrder list (TOOLS): patch floatThead + AJAX shell (Invoice / Opportunities pattern).
 */
(function ($) {
	'use strict';
	...
	function listConfig() {
		return window.__mkSoToolsListConfig || {};
	}
	...
	function applyListUi() {
		if (!isSalesOrderToolsList()) {
			return;
		}
		var $lv = getListViewContainer();
		$lv.addClass('mk-so-search-open mk-sales-list-table-ready mk-so-tools-list-ready');
	...
	function patchPlaceListContents() {
		...
		Vtiger_List_Js.prototype.placeListContents = function (contents) {
			if (isSalesOrderToolsList() && swapListBodyInShell(contents)) {
				applyListUi();
				return;
			}
			if (isSalesOrderToolsList()) {
	...
	function scheduleApply() {
		...
		$(window).off('load.mkSoToolsList').on('load.mkSoToolsList', applyListUi);
	}
	...
	window.applySalesOrderToolsListUi = applyListUi;
