{* SalesOrder list header (TOOLS) — same layout as Invoice / Opportunities *}
{strip}
<div class="mk-opportunity-header">
	<div class="mk-opportunity-breadcrumb" aria-label="Breadcrumb">
		...
		<span>{if $LISTVIEW_MODULE_TITLE}{$LISTVIEW_MODULE_TITLE}{else}{vtranslate('SalesOrder', 'SalesOrder')}{/if}</span>
	</div>
	<header class="mk-opportunity-action-header" role="region" aria-label="{vtranslate('SalesOrder', 'SalesOrder')}">
		<div class="mk-opportunity-action-header__text">
			<h1 class="mk-opportunity-action-header__title">{if $LISTVIEW_MODULE_TITLE}{$LISTVIEW_MODULE_TITLE}{else}{vtranslate('SalesOrder', 'SalesOrder')}{/if}</h1>
		...
		<div class="mk-opportunity-action-header__actions">
			...
			{if $ADD_ACTION}
				<button type="button" id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($ADD_ACTION->getLabel())}" class="mk-opportunity-btn mk-opportunity-btn--primary"
					...
					<span class="mk-opportunity-btn__txt">{if $LISTVIEW_ADD_RECORD_LABEL}{$LISTVIEW_ADD_RECORD_LABEL}{else}{vtranslate('LBL_ADD_RECORD', $MODULE)}{/if}</span>
				</button>
			{/if}
		</div>
	</header>
