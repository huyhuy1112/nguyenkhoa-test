{*+**********************************************************************************
* SalesOrder: SALES Detail tabs + Contacts SVG icons for related modules when app=SALES.
*************************************************************************************}

{strip}
{assign var=MK_SO_SALES_APP value=0}
{if (isset($SELECTED_MENU_CATEGORY) && $SELECTED_MENU_CATEGORY eq 'SALES') || (isset($smarty.get.app) && $smarty.get.app eq 'SALES')}
{assign var=MK_SO_SALES_APP value=1}
{/if}
	<div class='related-tabs row {if $MK_SO_SALES_APP}mk-so-detail-related-tabs{/if}'>
		<nav class="navbar margin0" role="navigation">
			<div class="navbar-header">
				<button type="button" class="navbar-toggle btn-group-justified collapsed border0" data-toggle="collapse" data-target="#nav-tabs" aria-expanded="false">
					{if $MK_SO_SALES_APP}<span class="mk-so-tab-more-svg mk-so-tab-more-svg--dots" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="19" cy="12" r="2"/></svg></span>{else}<i class="fa fa-ellipsis-h"></i>{/if}
				</button>
			</div>
...
{if $MK_SO_SALES_APP}
					<li class="mk-so-detail-tabs-spacer" role="presentation" aria-hidden="true"></li>
{/if}
                                            ...
                                            {for $i = 0 to $COUNT1-1}
                                                    ...
                                                    <li class="tab-item {if (trim($RELATED_LINK->getLabel())== trim($SELECTED_TAB_LABEL)) && ($RELATED_LINK->getId() == $SELECTED_RELATION_ID)}active{/if}" data-url="{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" data-label-key="{$RELATED_LINK->getLabel()}"
                                                            ...
                                                            <a href="index.php?{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" class="textOverflowEllipsis" displaylabel="{$DETAILVIEWRELATEDLINKLBL}" recordsCount="" >
                                                                    {if $MK_SO_SALES_APP}
                                                                    <span class="tab-icon mk-so-tab-icon-svg" aria-hidden="true">{include file="partials/ContactDetailTabSvgIcon.tpl"|@vtemplate_path:'Contacts' MODULE=$RELATEDMODULENAME}</span>
                                                                    {else}
                                                                    ...
                                                                    {for $j = $COUNT1 to $COUNT-1}
                                                                            ...
                                                                            <li class="more-tab {if (trim($RELATED_LINK->getLabel())== trim($SELECTED_TAB_LABEL)) && ($RELATED_LINK->getId() == $SELECTED_RELATION_ID)}active{/if}" data-url="{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" data-label-key="{$RELATED_LINK->getLabel()}"
                                                                                    ...
                                                                                    <a href="index.php?{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" displaylabel="{$DETAILVIEWRELATEDLINKLBL}" recordsCount="">
                                                                                            {if $MK_SO_SALES_APP}
                                                                                            <span class="tab-icon textOverflowEllipsis mk-so-tab-icon-row">
                                                                                                    <span class="mk-so-tab-icon-svg" aria-hidden="true">{include file="partials/ContactDetailTabSvgIcon.tpl"|@vtemplate_path:'Contacts' MODULE=$RELATEDMODULENAME}</span> &nbsp;<span class="content">{$DETAILVIEWRELATEDLINKLBL}</span>
                                                                                            </span>
                                                                                            {else}
                                                                                            {if $RELATEDMODULENAME eq "ModComments"}
                                                                                                <span class="tab-icon textOverflowEllipsis mk-so-tab-icon-row">
