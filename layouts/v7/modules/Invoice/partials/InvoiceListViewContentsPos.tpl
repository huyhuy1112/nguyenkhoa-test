{* Invoice SALES POS list table — same shell as SalesOrder, no add button in header *}
				...
				<thead>
					<tr class="listViewContentHeader">
						...
						{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
							...
							{include file="partials/InvoiceListMkColClass.tpl"|vtemplate_path:$MODULE FIELD_NAME=$LIST_HEADER_NAME}
							<th class="{$MK_SO_COL_CLASS}" {if isset($COLUMN_NAME) && $COLUMN_NAME eq $LISTVIEW_HEADER->get('name')} nowrap="nowrap" {/if}>
				...
				<tbody class="overflow-y">
					{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
						...
						<tr class="listViewEntries" data-id='{$DATA_ID}' data-recordUrl='{$DATA_URL}&app={$SELECTED_MENU_CATEGORY}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
							...
							{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
								...
								{include file="partials/InvoiceListMkColClass.tpl"|vtemplate_path:$MODULE FIELD_NAME=$LISTVIEW_HEADERNAME}
								<td class="listViewEntryValue {$MK_SO_COL_CLASS}" data-name="{$LISTVIEW_HEADER->get('name')}" title="{$LISTVIEW_ENTRY->getTitle($LISTVIEW_HEADER)}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}">
					...
					{if isset($LISTVIEW_ENTRIES_COUNT) && $LISTVIEW_ENTRIES_COUNT eq '0'}
						<tr class="emptyRecordsDiv">
							{* +2: checkbox/star control col; full colspan so empty text centers in card *}
							{assign var=COLSPAN_WIDTH value={php7_count($LISTVIEW_HEADERS)}+2}
							<td colspan="{$COLSPAN_WIDTH}" class="mk-inv-empty-td">
								<div class="emptyRecordsContent mk-inv-empty-msg" role="status">
