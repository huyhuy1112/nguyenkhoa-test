/**
 * Invoice list (SALES): enhance native Vtiger DOM; AJAX only swaps table body (keeps shell).
 */
(function ($) {
  "use strict";
  ...
  function listConfig() {
    return window.__mkInvSalesListConfig || {};
  }
  ...
  function isInvoiceSalesList() {
    var b = document.body;
    if (
      !b ||
      b.getAttribute("data-module") !== "Invoice" ||
  ...
  var POS_COL_CLASS_BY_FIELD = {
    ...
    invoice_no: "mk-so-col-order-no",
    subject: "mk-so-col-order-no",
    salesorder_id: "mk-so-col-customer",
  ...
  function bindInlineDrawerBridge() {
    if (document.documentElement.getAttribute("data-mk-inv-drawer-bridge")) {
      return;
    }
    document.documentElement.setAttribute("data-mk-inv-drawer-bridge", "1");
  ...
  function confirmSalesOrderWithWarehouse($panel, recordId, $btn) {
    ...
    function loadWarehouses() {
      ...
      $.ajax({
        ...
        type: "GET",
        dataType: "json",
        data: { module: "Invoice", action: "WarehouseList" },
      })
        .done(function (resp) {
  ...
  function massDeleteSalesOrders() {
    ...
    if (!ids.length) {
      if (app.helper && app.helper.showErrorNotification) {
        app.helper.showErrorNotification({
          message: "Chọn ít nhất 1 hóa đơn để xóa.",
        });
      } else {
        window.alert("Chọn ít nhất 1 hóa đơn để xóa.");
      }
      return;
    }
    var message =
      "Xóa " +
      ids.length +
      " hóa đơn đã chọn? Hành động này không hoàn tác.";
    var run = function () {
      var postData = {
        module: "Invoice",
        action: "MassDelete",
      ...
      app.request.post({ data: postData }).then(function (err) {
        ...
        if (err) {
          if (app.helper && app.helper.showErrorNotification) {
            app.helper.showErrorNotification({
              message: (err && err.message) || "Không xóa được hóa đơn.",
            });
          } else {
            window.alert((err && err.message) || "Không xóa được hóa đơn.");
          }
          return;
        }
        if (app.helper && app.helper.showSuccessNotification) {
  ...
  function applyPosColumnWidths($table) {
    ...
    var widthByClass = {
      ...
      "mk-so-col-star": "76px",
      "mk-so-col-order-no": "14%",
      "mk-so-col-time": "14%",
      "mk-so-col-customer": "16%",
      "mk-so-col-paid": "12%",
      "mk-so-col-status": "16%",
      "mk-so-col-due": "14%",
    };
    ...
    $headers.each(function () {
      ...
      if ($th.hasClass("mk-so-list-col-hidden")) {
        html += '<col class="mk-so-pos-col mk-so-list-col-hidden" style="width:0">';
        return;
      }
      var width = "auto";
      var matchedCls = "";
  ...
  function patchPlaceListContents() {
    ...
    Vtiger_List_Js.prototype.placeListContents = function (contents) {
      if (isInvoiceSalesList() && swapListBodyInShell(contents)) {
        syncPosFilterUi();
        applyListEnhancements();
        return;
      }
  ...
  window.applyInvoiceListUi = applyListEnhancements;
  window.mkInvMassDelete = massDeleteSalesOrders;
  window.mkInvMassDuplicate = massDuplicateSalesOrders;
  ...
  function boot() {
    ...
    whenVtigerListReady(function () {
      ...
      var listInstance =
        Vtiger_List_Js.getInstance && Vtiger_List_Js.getInstance();
      if (listInstance && isInvoiceSalesList()) {
        registerSalesPosRowClickEvent(listInstance);
      }
    });
  }
