/**
 * RecycleBin list (TOOLS): patch floatThead + AJAX shell (Invoice / Opportunities pattern).
 */
(function ($) {
	'use strict';
	...
	function listConfig() {
		return window.__mkRbToolsListConfig || {};
	}
	...
	function getListPageRoot($lv) {
		return $lv.find('.mk-rb-page.mk-opportunity-page, .mk-so-page.mk-opportunity-page').first();
	}
	...
	function applyListUi() {
		if (!isRecycleBinToolsList()) {
			return;
		}
		var $lv = getListViewContainer();
		$lv.addClass('mk-so-search-open mk-sales-list-table-ready mk-rb-list-ready');
	...
	function patchPlaceListContents() {
		...
		Vtiger_List_Js.prototype.placeListContents = function (contents) {
			if (isRecycleBinToolsList() && swapListBodyInShell(contents)) {
				applyListUi();
				return;
			}
			if (isRecycleBinToolsList()) {
	...
	function scheduleApply() {
		...
		$(window).off('load.mkRbToolsList').on('load.mkRbToolsList', applyListUi);
	}
	...
	window.applyRecycleBinToolsListUi = applyListUi;
