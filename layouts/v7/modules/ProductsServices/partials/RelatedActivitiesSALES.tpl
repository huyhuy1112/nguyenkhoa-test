	...
	<div class="summaryWidgetContainer">
		...
		<div class="widget_contents">
			{if php7_count($ACTIVITIES) neq '0'}
				...
				{/foreach}
			{else}
				<div class="noContent mk-ps-detail-activities-empty">
					<p class="textAlignCenter">{vtranslate('LBL_NO_PENDING_ACTIVITIES','Calendar')}</p>
				</div>
			{/if}
			{if $PAGING_MODEL->isNextPageExists()}
				<div class="row">
					<div class="textAlignCenter">
						<a href="javascript:void(0)" class="moreRecentActivities">{vtranslate('LBL_SHOW_MORE','Calendar')}</a>
					</div>
				</div>
			{/if}
		</div>
