...
{/if}
<div class="{$CLASS_VIEW_ACTION} mk-sc-pagination__btns">
	<div class="btn-group">
		<button type="button" id="PreviousPageButton" class="btn btn-default mk-sc-page-btn" {if !$PAGING_MODEL->isPrevPageExists()} disabled {/if}><i class="fa fa-caret-left"></i></button>
		{if $SHOWPAGEJUMP}
			<button type="button" id="PageJump" data-toggle="dropdown" class="btn btn-default mk-sc-page-btn">
		...
		{/if}
		<button type="button" id="NextPageButton" class="btn btn-default mk-sc-page-btn" {if !$PAGING_MODEL->isNextPageExists()}disabled{/if}><i class="fa fa-caret-right"></i></button>
	</div>
</div>
