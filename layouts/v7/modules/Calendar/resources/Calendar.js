...
Vtiger.Class(
  "Calendar_Calendar_Js",
  {
    ...
    getInstance: function () {
      if (!Calendar_Calendar_Js.calendarInstance) {
        if (app.view() == "SharedCalendar") {
          Calendar_Calendar_Js.calendarInstance =
            new Calendar_SharedCalendar_Js();
        } else {
    ...
    showLeaveRequestCreateModal: function (clickedDate) {
      var instance = Calendar_Calendar_Js.getInstance();
      instance.showLeaveRequestCreateModal(clickedDate);
    },
  ...
  },
  {
            ...
            if (e) {
              ...
              app.helper.showAlertNotification({
                message: app.vtranslate(
                  "JS_FUTURE_EVENT_CANNOT_BE_MARKED_AS_HELD"
                ),
              });
            }
          });
    ...
    registerHourFormatChangeEvent: function (modalContainer) {
      var hourFormatConditionMapping = jQuery(
        'input[name="timeFormatOptions"]',
        modalContainer
      ).data("value");
      ...
      form.find('input[name="hour_format"]').on("click", function () {
        ...
        var list =
          hourFormatConditionMapping["hour_format"][hourFormatVal][
            "start_hour"
          ];
        var options = "";
        for (var key in list) {
    ...
    disableFeed: function (sourceKey) {
      ...
      if (disabledFeeds.indexOf(sourceKey) === -1) {
        ...
        app.storage.set(
          Calendar_Calendar_Js.disabledFeedsStorageKey,
          disabledFeeds
        );
      }
    },
    ...
    renderEvents: function () {
      var thisInstance = this;
      this.getCalendarViewContainer().fullCalendar(
        "addEventSource",
        function (start, end, timezone, render) {
          ...
          activeFeeds.each(function () {
            ...
            var feedRequestParams = thisInstance.getFeedRequestParams(
              start,
              end,
              feedCheckbox
            );
            activeFeedsRequestParams[feedCheckbox.data("calendarSourcekey")] =
              feedRequestParams;
          });
    ...
    restoreFeedsState: function (widgetContainer) {
      var thisInstance = this;
      var disabledFeeds = this.getDisabledFeeds();
      var feedsList = widgetContainer.find(
        "#calendarview-feeds > ul.feedslist"
      );
    ...
    addEvents: function (feedCheckbox) {
      ...
      if (feedCheckbox.is(":checked")) {
        ...
        thisInstance.fetchEvents(feedCheckbox).then(
          function (events) {
            thisInstance
              .getCalendarViewContainer()
      ...
      this.getCalendarViewContainer().fullCalendar(
        ...
        function (eventObj) {
          return (
            ...
            eventObj.conditions === conditions &&
            fieldName === eventObj.fieldName
          );
        }
      );
    },
    ...
    updateRangeFields: function (container, options) {
      ...
      var optionsCount = fieldSelectElement
        .find("option")
        .not('option[value="birthday"]');

      if (moduleName === "Events" || moduleName === "Calendar") {
        optionsCount = fieldSelectElement
          .find("option")
          .not('option[value="date_start,due_date"]');
      }

      if (optionsCount.length > 1) {
        container
          .find('[name="rangeFields"]')
          .removeAttr("disabled")
          .trigger("change");
      } else {
        container
      ...
      var otherOption = targetFieldSelect
        .find("option")
        .not('option[value="' + selectedValue + '"]');
          ...
          for (var fieldName in moduleSpecificFields) {
            if (moduleSpecificFields.hasOwnProperty(fieldName)) {
              options +=
                ...
                fieldName +
                '">' +
                moduleSpecificFields[fieldName] +
                "</option>";
            }
          }
        }
    ...
    registerDateFieldChangeEvent: function (modalContainer) {
      ...
      fieldsSelect.on("change", function () {
        var moduleName = modalContainer
          .find('[name="modulesList"]')
        ...
        var feedCheckbox = jQuery(
          '[data-calendar-sourcekey="' + calendarSourceKey + '"]',
          parentElement
        );
        ...
        modalContainer
          .find(".calendarColorPicker")
          .ColorPickerSetColor(currentColor);
      });
    ...
    saveFeedSettings: function (modalContainer, feedIndicator) {
      ...
      if (editorMode === "create") {
        var translatedFieldName = modalContainer
          .find(".selectedType")
          .data("typename");
        if (modalContainer.find('[name="rangeFields"]').is(":checked")) {
          var sourceValue = modalContainer
            .find('[name="sourceFieldsList"]')
            .val();
          var targetValue = modalContainer
            .find('[name="targetFieldsList"]')
            .val();
          fieldName = sourceValue + "," + targetValue;
          translatedFieldName =
            modalContainer
              .find('[name="sourceFieldsList"] option:selected')
              .text() +
            "," +
            modalContainer
              .find('[name="targetFieldsList"] option:selected')
              .text();
        }
      }
      ...
      app.request.post({ data: params }).then(function (e, data) {
        if (!e) {
          ...
          var message = app.vtranslate(
            "JS_CALENDAR_VIEW_COLOR_UPDATED_SUCCESSFULLY"
          );
          ...
          if (parsedConditions.hasOwnProperty("value")) {
            ...
            feedIndicatorTitle =
              moduleName +
              "(" +
              app.vtranslate(parsedConditions.value) +
              ") -" +
              translatedFieldName;
          }

          if (editorMode === "create") {
            var translatedModuleName = modulesList
              .find("option:selected")
              .text();
            var feedIndicatorTemplate = jQuery("#calendarview-feeds").find(
              "ul.dummy > li.feed-indicator-template"
            );
            ...
            if (feedIndicatorModuleEndIndex == -1) {
              // ModuleName - title...
            ...
            jQuery("#calendarview-feeds")
              .find("ul:first")
              .append(feedIndicator);
    ...
    registerColorEditorSaveEvent: function (modalContainer, feedIndicator) {
      ...
      modalContainer.find('[name="saveButton"]').on("click", function () {
        ...
        selectedType
          .val(fieldName)
          .data("typename", fieldsSelect.find("option:selected").text());
        ...
        thisInstance
          .checkDuplicateFeed(moduleName, fieldName, selectedColor, conditions)
          .then(
            function (result) {
            ...
            function () {
              app.helper.showErrorNotification({
                message: app.vtranslate(
                  "JS_CALENDAR_VIEW_YOU_ARE_EDITING_NOT_FOUND"
                ),
              });
    ...
    registerColorEditorEvents: function (modalContainer, feedIndicator) {
      ...
      modulesSelect
        ...
        .select2("val", feedCheckbox.data("calendarFeed"))
        .trigger("change");

      var fieldSelectElement = modalContainer.find('[name="fieldsList"]');
      fieldSelectElement
        .select2("val", feedCheckbox.data("calendarFieldname"))
        .trigger("change");
    ...
    showColorEditor: function (feedIndicator) {
      ...
      app.request.post({ data: params }).then(function (e, data) {
        ...
        if (!e) {
          app.helper.showModal(data, {
            cb: function (modalContainer) {
              thisInstance.registerColorEditorEvents(
                modalContainer,
                feedIndicator
              );
            },
    ...
    registerFeedsColorEditEvent: function () {
      ...
      jQuery("#calendarview-feeds").on(
        "click",
        ".editCalendarFeedColor",
        function () {
          var feedIndicator = jQuery(this).closest(
            "li.calendar-feed-indicator"
          );
    ...
    registerAddActivityTypeEvent: function (modalContainer) {
      ...
      modalContainer.find('[name="saveButton"]').on("click", function () {
        ...
        selectedType
          .val(fieldSelect.val())
          .data("typename", fieldSelect.find("option:selected").text());
        var moduleName = modalContainer
          .find('select[name="modulesList"]')
          .val();
    ...
    registerAddActivityTypeFeedActions: function (modalContainer) {
      ...
      var sourceFieldsSelect = modalContainer.find(
        'select[name="sourceFieldsList"]'
      );
      sourceFieldsSelect.on("change", function () {
        var selectedValue = sourceFieldsSelect.find("option:selected").val();
        if (
          selectedValue === targetFieldsSelect.find("option:selected").val()
        ) {
          var otherOption = targetFieldsSelect
            .find("option")
            .not('option[value="' + selectedValue + '"]');
      ...
      var targetFieldsSelect = modalContainer.find(
        'select[name="targetFieldsList"]'
      );
      targetFieldsSelect.on("change", function () {
        var selectedValue = targetFieldsSelect.find("option:selected").val();
        if (
          selectedValue === sourceFieldsSelect.find("option:selected").val()
        ) {
          var otherOption = sourceFieldsSelect
            .find("option")
            .not('option[value="' + selectedValue + '"]');
      ...
      rangeFieldsOption.on("change", function () {
        ...
        var sourceSelectValue = sourceFieldsSelect
          .find("option:selected")
          .val();
        var otherOption = targetFieldsSelect
          .find("option")
          .not('option[value="' + sourceSelectValue + '"]');
    ...
    registerWidgetPostLoadEvent: function () {
      var thisInstance = this;
      app.event.on(
        Calendar_Calendar_Js.feedsWidgetPostLoadEvent,
        function (e, widgetContainer) {
          thisInstance.initializeMiniCalendar();
    ...
    registerFeedMassSelectEvent: function () {
      ...
      var calendarFeeds = jQuery(
        "ul.feedslist input.toggleCalendarFeed",
        container
      );
    ...
    initializeWidgets: function () {
      ...
      jQuery.each(widgets, function () {
        ...
        var dataUrl = widgetHeader.attr("data-url") || widgetHeader.data("url");
        if (!dataUrl) {
          widget.find(".sidebar-widget-body").html("");
          return;
        }
        var dataParams = app.convertUrlToDataParams(dataUrl);
        if (!dataParams || !dataParams.module) {
          widget.find(".sidebar-widget-body").html("");
          return;
        }
        ...
        app.request.post({ data: dataParams }).then(function (e, data) {
          if (!e) {
            widgetBody.html(typeof data === "string" ? data : data || "");
            var fullCalendarViewHeight = jQuery(".fc-view-container").length
              ? jQuery(".fc-view-container").first().height()
              : 0;
            var maxH =
              typeof fullCalendarViewHeight === "number" &&
              fullCalendarViewHeight > 20
                ? fullCalendarViewHeight - 10
                : 380;
            widgetBody.css("max-height", maxH + "px");
            app.helper.showVerticalScroll(widgetBody, {
            ...
            widgetBody.html(
              '<div class="text-muted small padding10">' +
                (e.message || "") +
                "</div>"
            );
          }
    ...
    getUserPrefered: function (setting) {
      if (
        typeof Calendar_Calendar_Js.userPreferenceCache[setting] === "undefined"
      ) {
        Calendar_Calendar_Js.userPreferenceCache[setting] = jQuery(
          "#" + setting
        ).val();
      }
    ...
    transformToEventObject: function (eventData, feedCheckbox) {
      ...
      eventObject.url =
        "index.php?module=Calendar&view=Detail&record=" + eventData._recordId;
            ...
            if (!err) {
              ...
              var message =
                typeof formData.record !== "undefined"
                  ? app.vtranslate("JS_EVENT_UPDATED")
    ...
    _updateAllOnCalendar: function (calendarModule) {
      ...
      this.getCalendarViewContainer().fullCalendar(
        "addEventSource",
        function (start, end, timezone, render) {
          var activeFeeds = jQuery(
            '[data-calendar-feed="' + calendarModule + '"]:checked'
          );
    ...
    updateEventOnResize: function (
      ...
      if (event.recurringcheck) {
        app.helper
          .showConfirmationForRepeatEvents()
    ...
    addActivityTypeIcons: function (event, element) {
      element
        .find(".fc-content > .fc-time")
        .prepend(
          "<span>" +
            '<i class="' +
            this.getActivityTypeClassName(event.activitytype) +
            '"></i>' +
            "</span>&nbsp;"
        );
    },
      ...
      app.request.post({ data: params }).then(function (e, res) {
        ...
        if (!e) {
          ...
          for (var key in deletedRecords) {
            ...
            thisInstance
              .getCalendarViewContainer()
              .fullCalendar("removeEvents", eventId);
          }
          app.helper.showSuccessNotification({
    ...
    showEditModal: function (moduleName, record, isRecurring) {
      ...
      if (quickCreateNode.length <= 0) {
        ...
        if (moduleName === "Events") {
          app.event.one("post.QuickCreateForm.show", function (e, form) {
            thisInstance.registerEditEventModalEvents(
              form.closest(".modal"),
              isRecurring
            );
          });
    ...
    registerPopoverEvent: function (event, element, calendarView) {
      ...
      var generatePopoverContentHTML = function (eventObj) {
        ...
        if (eventObj.module === "LeaveRequest") {
          ...
          if (
            eventObj._end &&
            eventObj._end.format(dateFormat) !== timeString
          ) {
            timeString += " → " + eventObj._end.format(dateFormat);
          }
        } else if (eventObj.activitytype === "Task") {
          timeString = moment(eventObj._start._i, eventObj._start._f).format(
            timeFormat
          );
    ...
    getCalendarConfigs: function () {
      ...
      var calenderConfigs = {
        ...
        firstDay:
          thisInstance.daysOfWeek[thisInstance.getUserPrefered("start_day")],
        ...
        events: [],
        // Ép parse start/end là giờ local để màu vẽ trên lịch khớp với form (tránh moment parse thành UTC)
        eventDataTransform: function (event) {
          ...
          if (event.start && typeof event.start === "string") {
            var m = moment(event.start, formats);
            if (m.isValid()) event.start = m;
          }
          if (event.end && typeof event.end === "string") {
            var m = moment(event.end, formats);
            if (m.isValid()) {
          ...
          // Giữ màu từ feed; chỉ thêm default nếu thiếu
          var bg = event.backgroundColor || event.color;
          if (!bg) {
            bg = event.module === "Calendar" ? "#2e7d32" : "#3f51b5";
            event.backgroundColor = bg;
            event.color = bg;
          }
          if (!event.textColor) event.textColor = "#ffffff";
          return event;
        },
        monthNames: [
        ...
        eventResize: function (event, delta, revertFunc, jsEvent, ui, view) {
          thisInstance.updateEventOnResize(
            event,
            delta,
            revertFunc,
            jsEvent,
        ...
        eventDrop: function (event, delta, revertFunc, jsEvent, ui, view) {
          thisInstance.updateEventOnDrop(
            event,
            delta,
            revertFunc,
            jsEvent,
        ...
        viewRender: function (view, element) {
          // Đồng bộ mini calendar sau khi lịch chính đã render (chỉ cần một lần)
          if (!thisInstance._miniCalendarSynced) {
            thisInstance._miniCalendarSynced = true;
            thisInstance.initializeMiniCalendar();
          }
    ...
    registerAgendaListView: function () {
      ...
      agendaListView = view.extend({
        ...
        renderHtml: function () {
          ...
          var dateFormat = this.vtDateFormat;
          var skeleton = "" + '<div class="agendaListView">';
          for (
            var i = 0;
            i < Calendar_Calendar_Js.numberOfDaysInAgendaView;
            i++
          ) {
            var daysToAdd = i ? 1 : 0;
            ...
            var part =
              "" +
              '<div class="agendaListDay" data-date="' +
              date +
              '">' +
        ...
        generateEventHTML: function (event) {
          var html =
            "" +
            '<div class="agenda-event-wrapper" data-event-id="' +
            event.id +
            '">' +
            ...
            '<i class="fa fa-plus-square-o" title=' +
            app.vtranslate("JS_DETAILS") +
            "></i>" +
            "</div>" +
            '<div class="agenda-event-title verticalAlignMiddle">&nbsp;' +
            '<i class="' +
            thisInstance.getActivityTypeClassName(event.activitytype) +
            '" title="' +
            app.vtranslate(event.activitytype) +
            '"></i>&nbsp;&nbsp;&nbsp;';
          if (event.recurringcheck) {
            html +=
              '<i class="fa fa-repeat" style="font-size:10px;" title="' +
              app.vtranslate("JS_RECURRING_EVENT") +
              '"></i>&nbsp;';
          }
          html +=
            this.getAgendaEventTitle(event) +
            "</div>" +
            '<div class="agenda-event-status verticalAlignMiddle">' +
            event.status +
            "</div>" +
    ...
    registerGotoDateButtonAction: function (navigationsContainer) {
      ...
      gotoButton
        ...
        .on("changeDate", function (e) {
          thisInstance
            .getCalendarViewContainer()
            .fullCalendar("gotoDate", moment(e.date));
        });
    },
    ...
    /** Minimap: mini calendar trong sidebar, CHỈ hiển thị đơn nghỉ phép (không full calendar) */
    initializeMiniCalendar: function () {
      var $mini = jQuery("#calendar-mini");
      if (!$mini.length) return;
      if ($mini.hasClass("fc")) return;
      var thisInstance = this;
      var mainCal = this.getCalendarViewContainer();
      try {
        $mini.fullCalendar({
          ...
          eventClick: function (calEvent) {
            if (
              calEvent.url &&
              calEvent.url.indexOf("LeaveRequestDetail") !== -1
            ) {
              thisInstance.showLeaveRequestDetailModal(
                calEvent.id
                  ? String(calEvent.id).replace("leaverequest_", "")
                  : ""
              );
          ...
          dateClick: function (date, allDay, jsEvent, view) {
            ...
            // Ấn thẳng vào ngày trên mini calendar → mở form tạo đơn nghỉ phép, điền sẵn ngày đó (gọi qua static để luôn có instance)
            Calendar_Calendar_Js.showLeaveRequestCreateModal(date);
          },
        });
        if (mainCal.length) {
          mainCal
            .off("viewRender.calendarMiniSync")
            .on("viewRender.calendarMiniSync", function (view) {
              var d = view.intervalStart;
              if (d && $mini.hasClass("fc")) $mini.fullCalendar("gotoDate", d);
            });
          var view = mainCal.fullCalendar("getView");
          if (view && view.intervalStart)
            $mini.fullCalendar("gotoDate", view.intervalStart);
        }
      } catch (err) {
        console.warn("Calendar mini init:", err);
      }
    },
    showLeaveRequestDetailModal: function (recordId) {
      if (!recordId) return;
      var thisInstance = this;
      var params = {
        module: app.getModuleName(),
        action: "LeaveRequestDetail",
        record: recordId,
      };
      app.request
        .get({ url: "index.php", data: params })
        .then(function (err, html) {
          ...
          var $wrapper = jQuery(
            '<div id="leave-request-detail-modal-wrapper"></div>'
          );
            ...
            app.request
              ...
              .then(function (err, res) {
                if (!err && res && res.success) {
                  var $mini = jQuery("#calendar-mini");
                  if ($mini.length && $mini.hasClass("fc"))
                    $mini.fullCalendar("refetchEvents");
                  var $main = thisInstance.getCalendarViewContainer();
                  if ($main.length && $main.hasClass("fc"))
          ...
          $modal.find(".btn-delete-leave").on("click", function () {
            ...
            if (!record) return;
            var msg =
              app.vtranslate("LBL_LEAVE_DELETE_CONFIRM") ||
                ...
                if (!err && res && res.success) {
                  ...
                  var m =
                    err && err.message
                      ...
                      ? res.error.message
                      : app.vtranslate("LBL_ERROR") || "Error";
                  alert(m);
                }
                $btn.prop("disabled", false);
              });
    ...
    showLeaveRequestCreateModal: function (clickedDate) {
      ...
      var requesterName =
        typeof _USERMETA !== "undefined" && _USERMETA.userlabel
          ? _USERMETA.userlabel
          : "";
      var modalHtml =
        '<div class="modal fade" tabindex="-1" role="dialog" id="leaveRequestCreateModal">' +
        ...
        L("LBL_LEAVE_REQUESTED_BY") +
        '</label><div class="col-sm-8"><p class="form-control-static leave-requester-name">' +
        (requesterName ? requesterName : "-") +
        "</p></div></div>" +
        '<div class="form-group"><label class="control-label col-sm-4">' +
        L("LBL_LEAVE_TYPE_DAY") +
        '</label><div class="col-sm-8"><select class="form-control leave-type"><option value="paid">F-' +
        L("LBL_LEAVE_TYPE_PAID") +
        '</option><option value="unpaid">F-' +
        L("LBL_LEAVE_TYPE_UNPAID") +
        "</option></select></div></div>" +
        '<div class="form-group"><label class="control-label col-sm-4">' +
        L("LBL_LEAVE_DATE") +
        '</label><div class="col-sm-8"><input type="text" class="form-control dateField leave-date-start" data-date-format="yyyy-mm-dd" placeholder="dd/mm/yyyy"> <span class="leave-date-arrow">→</span> <input type="text" class="form-control dateField leave-date-end" data-date-format="yyyy-mm-dd" placeholder="dd/mm/yyyy"></div></div>' +
        '<div class="form-group"><label class="control-label col-sm-4">' +
        L("LBL_LEAVE_HALF_DAY") +
        '</label><div class="col-sm-8"><input type="checkbox" class="leave-half-day"></div></div>' +
        '<div class="form-group"><label class="control-label col-sm-4">' +
        L("LBL_REQUESTED_DAYS") +
        '</label><div class="col-sm-8"><p class="form-control-static leave-requested-days">0 ' +
        L("LBL_LEAVE_DAYS") +
        "</p></div></div>" +
        '<div class="form-group"><label class="control-label col-sm-4">' +
        L("LBL_ADD_DESCRIPTION") +
        '</label><div class="col-sm-8"><textarea class="form-control leave-description" rows="3" placeholder="' +
        L("LBL_DESCRIPTION_PLACEHOLDER") +
        '"></textarea></div></div>' +
        "</div></div>" +
        '<div class="modal-footer"><button type="button" class="btn btn-primary btn-save-leave">' +
        L("LBL_SAVE") +
      ...
      if (
        ...
        typeof vtUtils.registerEventForDateFields === "function"
      ) {
        vtUtils.registerEventForDateFields($modal.find(".dateField"));
      } else if (typeof jQuery.fn.datepicker === "function") {
        $modal.find(".dateField").datepicker();
      }
      // Điền sẵn ngày khi click từ mini calendar
      if (clickedDate) {
        var dateStr = "";
        if (
          typeof clickedDate === "string" &&
          /^\d{4}-\d{2}-\d{2}/.test(clickedDate)
        ) {
          dateStr = clickedDate.substring(0, 10);
        } else if (clickedDate && typeof clickedDate.format === "function") {
          dateStr = clickedDate.format("YYYY-MM-DD");
        } else if (
          clickedDate &&
          clickedDate._d &&
          !isNaN(clickedDate._d.getTime())
        ) {
          var d = clickedDate._d;
          dateStr =
            d.getFullYear() +
            "-" +
          ...
          Object.prototype.toString.call(clickedDate) === "[object Date]" &&
        ...
        if (dateStr) {
          $modal.find(".leave-date-start").val(dateStr);
          $modal.find(".leave-date-end").val(dateStr);
        }
      }
      // Cập nhật "Đã đề nghị (ngày/giờ)?"
      ...
      function updateRequestedDays() {
        var start = $modal.find(".leave-date-start").val();
        var end = $modal.find(".leave-date-end").val() || start;
        var half = $modal.find(".leave-half-day").is(":checked");
        var days = 0;
        var d1 = parseDateStr(start);
        var d2 = parseDateStr(end);
        if (d1 && d2 && !isNaN(d1.getTime()) && !isNaN(d2.getTime())) {
          days = Math.round((d2 - d1) / 86400000) + 1;
          if (days < 1) days = 1;
          if (half && days === 1) days = 0.5;
          else if (half && days > 1) days = days - 0.5;
        }
        var label =
          (days === 0.5 ? "0.5" : String(days)) + " " + L("LBL_LEAVE_DAYS");
        $modal.find(".leave-requested-days").text(label);
      }
      $modal
        .find(".leave-date-start, .leave-date-end, .leave-half-day")
        .on("change input", updateRequestedDays);
      updateRequestedDays();
      $modal.find(".btn-save-leave").on("click", function () {
        var dateStart = $modal.find(".leave-date-start").val();
        if (!dateStart) {
          alert(L("LBL_PLEASE_SELECT_DATE"));
          return;
        }
        var dateEnd = $modal.find(".leave-date-end").val() || dateStart;
        var payload = {
          module: app.getModuleName(),
          action: "SaveLeaveRequest",
          subject: "",
          leave_type: $modal.find(".leave-type").val(),
          half_day: $modal.find(".leave-half-day").is(":checked") ? 1 : 0,
          date_start: dateStart,
          due_date: dateEnd,
          description: $modal.find(".leave-description").val(),
        };
        var $btn = jQuery(this).prop("disabled", true);
        app.request
          .post({ url: "index.php", data: payload })
          .then(function (err, res) {
            $btn.prop("disabled", false);
    ...
    registerEvents: function () {
      this._super();
      // Event delegation: bấm vào ô ngày mini calendar → mở form nghỉ phép
      jQuery(document)
        .off("click.calendarMiniLeave", "#calendar-mini-wrap .fc-day")
        .on(
          "click.calendarMiniLeave",
          "#calendar-mini-wrap .fc-day",
          function (e) {
            var $cell = jQuery(e.currentTarget);
            var dateStr =
              $cell.attr("data-date") ||
              $cell.closest("[data-date]").attr("data-date");
            if (dateStr && /^\d{4}-\d{2}-\d{2}/.test(dateStr)) {
              e.preventDefault();
              e.stopPropagation();
              Calendar_Calendar_Js.showLeaveRequestCreateModal(
                dateStr.substring(0, 10)
              );
            }
          }
        );
      // Đảm bảo nav không dùng mCustomScrollbar (tránh chặn kéo/chọn lịch)
      var $navMenu = jQuery("#modnavigator #modules-menu");
      if ($navMenu.length && typeof $navMenu.mCustomScrollbar === "function") {
        try {
          $navMenu.mCustomScrollbar("destroy");
        } catch (e) {}
      }
      this.initializeCalendar();
      this.initializeMiniCalendar();
      // Khởi tạo lại mini calendar sau khi sidebar/widget sẵn sàng (đảm bảo hiển thị kế bên full calendar)
      var thisInstance = this;
      setTimeout(function () {
        thisInstance.initializeMiniCalendar();
      }, 300);
      ...
      // Fallback: nếu widget Activity Types vẫn hiển thị loading sau 8s thì thay bằng thông báo
      setTimeout(function () {
        jQuery(".sidebar-widget-body").each(function () {
          var body = jQuery(this);
          if (body.find('img[src*="loading.gif"]').length) {
            body.html(
              '<div class="text-muted small padding10">' +
                (app.vtranslate("LBL_LOADING_FAILED") ||
