{* Create / Edit Calendar Task — MANAGEMENT shell + stock #EditView fields. *}
{strip}
{assign var=MK_CAL_URL value=$MK_CALENDAR_URL|default:'index.php?module=Calendar&view=Calendar&app=MANAGEMENT'}
<div class="mk-event-create mk-calendar-create" id="mkCalendarCreateWorkspace" data-mk-activity-create="1">
	<header class="mk-event-page-head">
		...
		<div class="mk-event-page-head__row">
			<div>
				{if !empty($RECORD_ID)}
					<h1 class="mk-event-page-head__title">{vtranslate('LBL_EDITING', $MODULE)} {vtranslate('SINGLE_Calendar', $MODULE)}</h1>
					{if !empty($RECORD_STRUCTURE_MODEL)}
						<p class="mk-event-page-head__sub">{$RECORD_STRUCTURE_MODEL->getRecordName()|escape}</p>
					{/if}
				{else}
					<h1 class="mk-event-page-head__title">{vtranslate('LBL_CREATING_NEW', $MODULE)} {vtranslate('SINGLE_Calendar', $MODULE)}</h1>
					<p class="mk-event-page-head__sub">{vtranslate('LBL_TASK_INFORMATION', $MODULE)}</p>
				{/if}
			</div>
	...
	<div class="mk-event-create-body">
		<div class="mk-event-form-host" id="mkCalendarFormHost">
