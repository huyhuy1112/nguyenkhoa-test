{* Campaigns: MARKETING Detail — primary tabs + collapsible related icon row. *}
{strip}
	{assign var=MK_CAMP_MK_TABS value=((isset($SELECTED_MENU_CATEGORY) && $SELECTED_MENU_CATEGORY eq 'MARKETING') || (isset($smarty.get.app) && $smarty.get.app eq 'MARKETING'))}
	<div class="related-tabs row {if $MK_CAMP_MK_TABS}mk-camp-detail-related-tabs{/if}">
				...
				<ul class="nav nav-tabs mk-sales-primary-tabs">
					{foreach item=RELATED_LINK from=$DETAILVIEW_LINKS['DETAILVIEWTAB']}
						...
						<li class="tab-item {if $RELATED_TAB_LABEL==$SELECTED_TAB_LABEL}active{/if}" data-url="{$RELATEDLINK_URL}&tab_label={$RELATED_TAB_LABEL}&app={$SELECTED_MENU_CATEGORY}" data-label-key="{$RELATEDLINK_LABEL}" data-link-key="{$RELATED_LINK->get('linkKey')}" >
							<a href="{$RELATEDLINK_URL}&tab_label={$RELATEDLINK_LABEL}&app={$SELECTED_MENU_CATEGORY}" class="{if !$MK_CAMP_MK_TABS}textOverflowEllipsis{/if} mk-sales-primary-tab-link">
