{* Potentials list toolbar — no decorative grid/sort icons; search + columns + mass actions only. *}
{strip}
{if !isset($MK_SALES_LIST_COUNT_SUFFIX) || $MK_SALES_LIST_COUNT_SUFFIX eq ''}
	{assign var=MK_SALES_LIST_COUNT_SUFFIX value=' opportunities'}
{/if}
{assign var=PRINT_TEMPLATE value=false}
{assign var=LISTVIEW_MASSACTIONS_1 value=array()}
<div id="listview-actions" class="listview-actions-container mk-so-filter-row mk-opportunity-filter-row">
	{foreach item=LIST_MASSACTION from=$LISTVIEW_MASSACTIONS name=massActions}
		...
		{/if}
	{/foreach}
	<div class="mk-so-filter-row__inner mk-opportunity-filter-row__inner">
		<div class="mk-so-filter-row__start mk-opportunity-filter-row__start">
			<div class="mk-so-toolbar-count mk-opportunity-toolbar-count">
				{assign var=RECORD_COUNT value=$LISTVIEW_ENTRIES_COUNT}
				<span class="pageNumbers mk-so-page-numbers mk-opportunity-page-numbers">
		...
		<div class="btn-group listViewActionsContainer mk-so-mass-actions mk-opportunity-mass-actions" role="group" aria-label="{vtranslate('LBL_ACTIONS',$MODULE)}">
			{if isset($editAction) && $editAction}
				<button type="button" class="btn btn-default mk-so-mass-btn mk-opportunity-mass-btn" id={$MODULE}_listView_massAction_{$editAction->getLabel()}
			...
			{if isset($deleteAction) && $deleteAction}
				<button type="button" class="btn btn-default mk-so-mass-btn mk-opportunity-mass-btn" id={$MODULE}_listView_massAction_{$deleteAction->getLabel()}
			...
			{if isset($commentAction) && $commentAction}
				<button type="button" class="btn btn-default mk-so-mass-btn mk-opportunity-mass-btn" id="{$MODULE}_listView_massAction_{$commentAction->getLabel()}"
			...
			{if php7_count($LISTVIEW_MASSACTIONS_1) gt 0 or $LISTVIEW_LINKS['LISTVIEW']|@count gt 0}
				<div class="btn-group listViewMassActions" role="group">
					<button type="button" class="btn btn-default btn-sm dropdown-toggle mk-so-mass-more mk-opportunity-mass-more" data-toggle="dropdown">
						{vtranslate('LBL_MORE','Vtiger')}&nbsp;<span class="caret"></span>
					</button>
					<ul class="dropdown-menu" role="menu">
						...
						{if $MODULE_MODEL->isStarredEnabled()}
							<li class="hide"><a id="{$MODULE}_listView_massAction_LBL_ADD_STAR" onclick="Vtiger_List_Js.triggerAddStar()">{vtranslate('LBL_FOLLOW',$MODULE)}</a></li>
							<li class="hide"><a id="{$MODULE}_listView_massAction_LBL_REMOVE_STAR" onclick="Vtiger_List_Js.triggerRemoveStar()">{vtranslate('LBL_UNFOLLOW',$MODULE)}</a></li>
						{/if}
						<li class="hide"><a id="{$MODULE}_listView_massAction_LBL_ADD_TAG" onclick="Vtiger_List_Js.triggerAddTag()">{vtranslate('LBL_ADD_TAG',$MODULE)}</a></li>
						{if $CURRENT_TAG neq ''}
							<li class="hide"><a id="{$MODULE}_listview_massAction_LBL_REMOVE_TAG" onclick="Vtiger_List_Js.triggerRemoveTag({$CURRENT_TAG})">{vtranslate('LBL_REMOVE_TAG', $MODULE)}</a></li>
						{/if}
						...
						{/if}
						{foreach item=LISTVIEW_ADVANCEDACTIONS from=$LISTVIEW_LINKS['LISTVIEW']}
							{if $LISTVIEW_ADVANCEDACTIONS->getLabel() != 'LBL_IMPORT' && $LISTVIEW_ADVANCEDACTIONS->getLabel() != 'Print' && $LISTVIEW_ADVANCEDACTIONS->getLabel() != 'LBL_FIND_DUPLICATES'}
		...
		{* Grid + magnifying-glass toolbar icons hidden — use in-table Search row + header column picker *}
	</div>
