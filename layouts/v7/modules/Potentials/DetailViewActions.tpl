{* Potentials Detail actions: Leads v4 iconbtn style (1 box = icon+label). *}
{strip}
{if (isset($SELECTED_MENU_CATEGORY) && $SELECTED_MENU_CATEGORY eq 'SALES') || (isset($smarty.get.app) && $smarty.get.app eq 'SALES')}
	...
	<div class="{$MK_OPP_ACTION_COL}">
		<div class="mk-opportunity-detail-actions__toolbar">
			<div class="mk-opportunity-detail-actions__group">
			{assign var=STARRED value=$RECORD->get('starred')}
			{if $MODULE_MODEL->isStarredEnabled()}
				<button type="button" class="btn btn-default mk-opportunity-detail-btn mk-opportunity-detail-btn--icon markStar {if $STARRED} active {/if}" id="starToggle" title="{vtranslate('LBL_FOLLOW',$MODULE)}">
			...
			{foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
				...
				<button type="button" class="btn btn-default mk-opportunity-detail-btn mk-opportunity-detail-btn--icon" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($MK_BASIC_LBL)}"
						title="{vtranslate($MK_BASIC_LBL, $MODULE_NAME)}"
			...
			<button type="button" class="btn btn-default mk-opportunity-detail-btn mk-opportunity-detail-btn--icon" id="mkOppCallCustomerBtn" title="Gọi khách hàng">
				<span class="mk-opportunity-detail-btn__ic" aria-hidden="true">{include file="partials/OpportunityDetailSvgIcon.tpl"|@vtemplate_path:$MODULE ICON='PHONE'}</span>
				<span class="mk-opportunity-detail-btn__txt">Gọi</span>
			</button>

			{if !empty($DETAILVIEW_LINKS['DETAILVIEW']) && ($DETAILVIEW_LINKS['DETAILVIEW']|@count gt 0)}
				<div class="btn-group mk-opportunity-detail-more">
					<button type="button" class="btn btn-default mk-opportunity-detail-btn mk-opportunity-detail-btn--icon dropdown-toggle" data-toggle="dropdown" href="javascript:void(0);" title="{vtranslate('LBL_MORE', $MODULE_NAME)}">
			...
			<button type="button" class="btn mk-opportunity-detail-btn mk-opportunity-detail-btn--cta" id="mkOppConvertToCustomerBtn" title="Chuyển sang Khách hàng">
				<span class="mk-opportunity-detail-btn__ic" aria-hidden="true">{include file="partials/OpportunityDetailSvgIcon.tpl"|@vtemplate_path:$MODULE ICON='CONVERT'}</span>
