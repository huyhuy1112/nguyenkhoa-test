{* Quotes SALES POS list table — match SalesOrder POS markup *}
				...
				<thead>
					<tr class="listViewContentHeader">
						...
						{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
							...
							{include file="partials/SalesListMkQtColClass.tpl"|vtemplate_path:$MODULE FIELD_NAME=$LIST_HEADER_NAME}
							<th class="{$MK_QT_COL_CLASS}" {if isset($COLUMN_NAME) && $COLUMN_NAME eq $LISTVIEW_HEADER->get('name')} nowrap="nowrap" {/if}>
								<a href="#" class="{if $NO_SORTING}noSorting{else}listViewContentHeaderValues{/if}" {if !$NO_SORTING}data-nextsortorderval="{if $COLUMN_NAME eq $LISTVIEW_HEADER->get('name')}{$NEXT_SORT_ORDER}{else}ASC{/if}" data-columnname="{$LISTVIEW_HEADER->get('name')}"{/if} data-field-id='{$LISTVIEW_HEADER->getId()}'>
									...
									{if $LIST_HEADER_NAME eq 'quotestage'}
					...
					{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
						...
						<tr class="listViewEntries" data-id='{$DATA_ID}' data-recordUrl='{$DATA_URL}&app={$SELECTED_MENU_CATEGORY}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
							...
							{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
								...
								{include file="partials/SalesListMkQtColClass.tpl"|vtemplate_path:$MODULE FIELD_NAME=$LISTVIEW_HEADERNAME}
								<td class="listViewEntryValue {$MK_QT_COL_CLASS}" data-name="{$LISTVIEW_HEADER->get('name')}" title="{$LISTVIEW_ENTRY->getTitle($LISTVIEW_HEADER)}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}">
