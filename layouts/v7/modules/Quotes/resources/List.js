...
(function ($) {
	...
	var TEXT_DECODE_FIELDS = ['account_id', 'accountname', 'potential_id', 'subject', 'contact_id'];
	...
	function fixCurrencySpacing($table) {
		...
		$table.find('tbody td[data-name="total"] .value, tbody td[data-name="hdnGrandTotal"] .value').each(function () {
			var $node = $(this);
	...
	function buildInlineExportPreviewHtml($panel) {
		...
		$panel.find('.mk-so-inline-detail__total-row').each(function () {
			...
			// Preview mirrors invoice sample labels.
			if (label.indexOf('Giảm giá') === 0) {
				label = 'Chiết khấu';
			} else if (label.indexOf('Tổng cộng') === 0) {
		...
		html += '<div class="mk-so-excel-sheet__doc-meta">Mã báo giá: ' + esc(quoteNo) + '</div>';
		if (createdDate) {
		...
		if (!lines.length) {
			html += '<tr><td colspan="3" class="is-empty">Chưa có hàng hóa trong báo giá.</td></tr>';
		} else {
	...
	function downloadInlineExportXlsx($panel, recordId) {
		...
		if (!excelUrl) {
			excelUrl = 'index.php?module=Quotes&action=ExportExcelForSale&record=' + encodeURIComponent(recordId);
		}
		var $frame = $('#mk-qt-inline-excel-download-frame');
		if (!$frame.length) {
			$frame = $('<iframe id="mk-qt-inline-excel-download-frame" class="mk-so-inline-print-download-frame" title="Tải Excel báo giá"></iframe>');
			$('body').append($frame);
		}
	...
	function openInlineExcelPreview($panel, recordId) {
		...
		$modal.off('click.mkQtExcelDownload').on('click.mkQtExcelDownload', '.mk-so-inline-excel-preview__download', function (e) {
			e.preventDefault();
	...
	function getInlinePrintPreviewUrl($panel, recordId) {
		...
		if (!printUrl) {
			printUrl = 'index.php?module=Quotes&action=ExportPDF&record=' + encodeURIComponent(recordId) + '&preview=1';
		}
		return printUrl;
	}
	...
	function getInlinePrintDownloadUrl($panel, recordId) {
		...
		if (!downloadUrl) {
			downloadUrl = 'index.php?module=Quotes&action=ExportPDF&record=' + encodeURIComponent(recordId);
		}
		return downloadUrl;
	}
	...
	function downloadInlinePrintPdf($panel, recordId) {
		...
		if (!$frame.length) {
			$frame = $('<iframe id="mk-qt-inline-print-download-frame" class="mk-so-inline-print-download-frame" title="Tải PDF báo giá"></iframe>');
			$('body').append($frame);
		}
	...
	function initInlineDetailPanel($container) {
		...
		$panel.on('click', '.mk-so-inline-detail__view-full-btn', function (e) {
			...
			if (!detailUrl) {
				detailUrl = 'index.php?module=Quotes&view=Detail&record=' + encodeURIComponent(recordId) + '&app=SALES';
			}
		...
		$panel.on('click', '.mk-so-inline-detail__process-btn', function (e) {
			...
			if (!editUrl) {
				editUrl = 'index.php?module=Quotes&view=Edit&record=' + encodeURIComponent(recordId) + '&app=SALES';
			}
	...
	function patchInlineDetailRowClick() {
		if (typeof Vtiger_List_Js === 'undefined' || Vtiger_List_Js.prototype._mkQtInlineDetailPatched) {
			return;
		}
	...
	function bindInlineDetailCapture() {
		if (document.documentElement.getAttribute('data-mk-qt-inline-detail-bound')) {
			return;
		}
		document.documentElement.setAttribute('data-mk-qt-inline-detail-bound', '1');
	...
	function bindPosToolbarEvents() {
		...
		$(document)
			...
			.on('click.mkQtPosColumns', '.mk-qt-pos-trigger-columns', function (e) {
				if (!isQuotesSalesList()) return;
				e.preventDefault();
				$('#listViewContent').find('.listColumnFilter').first().trigger('click');
			});
	}
	...
	function injectSummaryRow($table) {
		...
		var dueField = $table.find('thead a[data-columnname="hdnGrandTotal"]').length
			? 'hdnGrandTotal'
			: ($table.find('thead a[data-columnname="total"]').length ? 'total' : '');
		...
		var hasDueColumn = dueField && $headerRow.find('a[data-columnname="' + dueField + '"]').length > 0;
		var dueTotal = 0;
		if (hasDueColumn) {
			$rows.each(function () {
				dueTotal += cellMoneyAmount($(this).find('td[data-name="' + dueField + '"]').first());
			});
		}
