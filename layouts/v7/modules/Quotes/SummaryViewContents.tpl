{* Quotes SummaryViewContents: SALES shows curated key fields only; other apps unchanged. *}
{strip}
...
<table class="summary-table no-border mk-qt-detail-summary-table">
	...
	{foreach item=MK_FNAME from=$MK_QT_SUMMARY_KEYS}
		...
		{if $FIELD_MODEL->getPermissions()}
			...
			{assign var=FIELD_MODEL value=$FIELD_MODEL->set('fieldvalue', $RECORD->get($MK_FNAME))}
			<tr class="summaryViewEntries">
				...
				<td class="fieldValue">
					<div class="">
						...
						<span class="value textOverflowEllipsis{if $MK_FNAME eq 'hdnGrandTotal'} mk-qt-summary-currency{/if}" title="{strip_tags($DISPLAY_VALUE)}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20' or $FIELD_MODEL->get('uitype') eq '21'}style="word-wrap: break-word;"{/if}>
