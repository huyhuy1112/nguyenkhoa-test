/**
 * SALES app list — shared toolbar footer (mk-so-filter-row__footer below table), floatThead off, AJAX shell.
 */
(function ($) {
	'use strict';
	...
	function patchShowPagingInfo() {
		...
		Vtiger_List_Js.prototype.showPagingInfo = function () {
			...
			if (!isSalesAppList() || !listViewContainer.find('.mk-so-page-numbers').length) {
	...
	function patchPlaceListContents() {
		...
		Vtiger_List_Js.prototype.placeListContents = function (contents) {
			if (isSalesAppList() && swapListBodyInShell(contents)) {
				applyCommonUi();
				if (typeof window.applySalesOrderListUi === 'function') {
