...
{assign var=PHONE_DISPLAY value=$FIELD_MODEL->getDisplayValue($FIELD_VALUE, $RECORD->getId(), $RECORD)}
{if $MODULEMODEL and $MODULEMODEL->isActive() and $FIELD_VALUE}
    ...
    {if $PERMISSION}
        ...
        <a class="phoneField" data-value="{$PHONE_NUMBER}" record="{$RECORD->getId()}" onclick="Vtiger_PBXManager_Js.registerPBXOutboundCall('{$PHONE_NUMBER}',{$RECORD->getId()})">{$PHONE_DISPLAY}</a>
    {else}
        {$PHONE_DISPLAY}
    {/if}
{else}
