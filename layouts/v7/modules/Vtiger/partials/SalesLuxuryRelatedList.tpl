...
{strip}
	...
	<div class="relatedContainer">
		...
		{include file="partials/SalesLuxuryRelatedListHeader.tpl"|vtemplate_path:'Vtiger'}
					...
					{foreach item=RELATED_RECORD from=$RELATED_RECORDS}
						<tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' 
							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								...
								<td class="relatedListEntryValues{if $RELATED_HEADERNAME eq 'time_start' or $RELATED_HEADERNAME eq 'time_end'} mk-rel-spacer-col{/if}" title="{strip_tags((isset($RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)))?$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME):"")}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
										...
										{if $RELATED_MODULE_NAME eq 'Documents' && $RELATED_HEADERNAME eq 'document_source'}
											...
											{elseif $RELATED_HEADERNAME eq 'recurringtype' && ($RELATED_MODULE_NAME eq 'Calendar' || $RELATED_MODULE_NAME eq 'Events')}
												{assign var=RECURRING_DETAILS value=$RELATED_RECORD->getRecurringDetails()}
												{if $RECURRING_DETAILS.recurringcheck eq 'Yes' && !empty($RECURRING_DETAILS.recurringenddate)}
													{$RECURRING_DETAILS.recurringenddate}
												{/if}
