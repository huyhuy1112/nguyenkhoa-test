...
jQuery.Class(
  "Vtiger_Helper_Js",
  {
    ...
    requestToShowComposeEmailForm: function (
      selectedId,
      fieldname,
      fieldmodule
    ) {
    ...
    getInternalMailer: function (selectedId, fieldname, fieldmodule) {
      ...
      var checkServerConfigPostOperations = function (data) {
        if (data == true) {
          Vtiger_Helper_Js.requestToShowComposeEmailForm(
            selectedId,
            fieldname,
            fieldmodule
          );
      ...
      if (cacheResponse === "") {
        ...
        checkServerConfig.then(function (data) {
          ...
          checkServerConfigPostOperations(
  ...
  },
  {
    ...
    checkServerConfig: function (module) {
      ...
      app.request.post({ data: actionParams }).then(function (err, data) {
				var state = false;
    ...
    showConfirmationBox: function (data) {
      ...
      if (
        ...
        Object.keys(data.buttons).length > 0
      ) {
    ...
    loadPageOverlay: function (data, params) {
      ...
      $("#overlayPage").one("shown.bs.modal", function () {
        if (
          !params.hasOwnProperty("ignoreScroll") &&
          params["ignoreScroll"] != true
        ) {
                var scrollParams = {
            height: "",
            railVisible: true,
      ...
      jQuery(window).one("resize", function () {
        var max_height =
          jQuery(window).height() - $(".global-nav").height() - 60;
        $("#overlayPage")
    ...
    loadPageContentOverlay: function (data, params) {
      ...
      if (
        ...
        (jQuery(".settingsgroup").length === 0 &&
          jQuery("#modules-menu").length === 0)
      ) {
    ...
    */ 
    ...
    _initNotificationDefaults: function () {
      if (jQuery(".module-action-bar").length) {
          ...
          z_index: 10003,
          template:
            '<div data-notify="container" class="col-xs-11 col-sm-3 vt-notification vt-notification-{0}" role="alert">' +
    ...
    dynamicListViewHorizontalScroll: function () {
      ...
      var tableHeight = jQuery(".sticky-wrap").height();
        ...
        jQuery(".sticky-wrap").scrollLeft(
          jQuery("#scroller_wrapper").scrollLeft()
        );
            });
    ...
    dynamicPopupViewHorizontalScroll: function () {
      ...
      var winHeight = jQuery(".modal-body").innerHeight();
            ...
            jQuery("#scroller_wrapper").scroll(function () {
        jQuery(".popupEntriesDiv").scrollLeft(
          jQuery("#scroller_wrapper").scrollLeft()
        );
            });
      ...
      jQuery(".modal .close,.modal .cancelLink").click(function (e) {
        if (
          ...
          app.helper
            .showConfirmationBox({
              message:
                app.vtranslate("JS_CHANGES_WILL_BE_LOST") +
                " " +
                app.vtranslate("JS_WISH_TO_PROCEED"),
            })
            .then(function () {
					window.onbeforeunload = null;
